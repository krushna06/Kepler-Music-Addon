...
module.exports = async (client) => {
  ...
  client.playembed = (guild) => {
    let embed = new EmbedBuilder()
      ...
      .setDescription(
        `[Support Server](${client.config.links.DiscordServer}) • [Website](${client.config.links.Website})`
      )
      .setImage(
        guild.banner
    ...
    queueembed.edit({
      embeds: [
        new EmbedBuilder()
          .setColor(client.config.embed.color)
          .setAuthor({
            name: `KEPLER Queue  -  [ ${queue.songs.length} Tracks ]`,
            iconURL: guild.iconURL({ dynamic: true }),
          })
